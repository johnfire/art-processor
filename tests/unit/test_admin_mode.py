...
class TestRunMenuLoop:
    ...
    def test_exit_immediately(self, admin):
        with patch("src.app.services.admin_mode.IntPrompt.ask", return_value=0):
            admin.run()

    def test_dispatches_to_correct_method(self, admin):
        # Choose option 7 (view settings) then 0 (exit)
        with patch("src.app.services.admin_mode.IntPrompt.ask", side_effect=[7, 0]), \
...
class TestEditApiKey:
    ...
    def test_update_existing_key(self, admin, env_file):
        with patch("src.app.services.admin_mode.Prompt.ask", return_value="sk-ant-newkey999"):
            admin.edit_api_key()
    ...
    def test_keep_existing_key(self, admin, env_file):
        with patch("src.app.services.admin_mode.Prompt.ask", return_value=""):
            admin.edit_api_key()
    ...
    def test_create_env_if_missing(self, settings_file, tmp_path):
        ...
        with patch("src.app.services.admin_mode.Confirm.ask", return_value=True), \
             patch("src.app.services.admin_mode.Prompt.ask", return_value="sk-ant-brand-new"):
            admin.edit_api_key()
    ...
    def test_no_create_env_if_declined(self, settings_file, tmp_path):
        ...
        with patch("src.app.services.admin_mode.Confirm.ask", return_value=False):
            admin.edit_api_key()
...
class TestEditFilePaths:
    ...
    def test_update_path(self, admin, env_file):
        ...
        with patch("src.app.services.admin_mode.Prompt.ask", side_effect=["/new/big", "", ""]):
...
class TestEditDimensionUnit:
    ...
    def test_switch_to_inches(self, admin, settings_file):
        with patch("src.app.services.admin_mode.IntPrompt.ask", return_value=2):
    ...
    def test_keep_cm(self, admin, settings_file):
        with patch("src.app.services.admin_mode.IntPrompt.ask", return_value=1):
            admin.edit_dimension_unit()
...
class TestAddToList:
    ...
    def test_add_substrate(self, admin, settings_file):
        with patch("src.app.services.admin_mode.Prompt.ask", side_effect=["1", "Wood"]):
            admin.add_to_list()
    ...
    def test_add_collection(self, admin, settings_file):
        with patch("src.app.services.admin_mode.Prompt.ask", side_effect=["5", "New Collection"]):
            admin.add_to_list()
    ...
    def test_back_to_menu(self, admin, settings_file):
        ...
        with patch("src.app.services.admin_mode.Prompt.ask", return_value="0"):
            admin.add_to_list()
    ...
    def test_empty_entry(self, admin, settings_file):
        ...
        with patch("src.app.services.admin_mode.Prompt.ask", side_effect=["1", ""]):
            admin.add_to_list()
...
class TestWrapperMethods:
    ...
    def test_edit_metadata_calls_cli(self, admin):
        with patch("src.app.services.admin_mode.edit_metadata_cli", create=True) as mock_cli, \
             patch("src.app.services.admin_mode.Prompt.ask", return_value=""), \
             patch("src.app.services.metadata_editor.edit_metadata_cli", return_value=None):
            admin.edit_metadata()

    def test_sync_instagram_calls_cli(self, admin):
        with patch("src.app.services.instagram_folder_sync.sync_instagram_folders_cli", return_value={}), \
             patch("src.app.services.admin_mode.Prompt.ask", return_value=""):
