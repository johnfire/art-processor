...
from src.social.flickr import FlickrPlatform
...
def post_to_flickr(ctx, title, description):
    ...
    if "mock_upload_error" in ctx:
        ...
        with patch("src.social.flickr.urlopen", return_value=upload_resp):
        ...
        with patch("src.social.flickr.urlopen", side_effect=[upload_resp, login_resp]):
            result = flickr.post_image(image_path, description, title)
    else:
        result = flickr.post_image(image_path, description, title)
...
def verify_flickr_credentials(ctx):
    ...
    if "mock_nsid" in ctx:
        ...
        login_resp = _mock_response(login_json)
        with patch("src.social.flickr.urlopen", return_value=login_resp):
